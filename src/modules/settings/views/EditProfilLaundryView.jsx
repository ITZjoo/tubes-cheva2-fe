...
function TimeField({ label, value, onChange }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex h-[52px] w-[176px] items-center gap-3 rounded-2xl border border-outline-variant bg-on-primary px-4">
        <input
          ref={inputRef}
          type="time"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="w-[64px] flex-1 self-center bg-transparent text-[18px] font-medium leading-none text-on-surface outline-none [&::-webkit-calendar-picker-indicator]:hidden"
        />
        <span aria-hidden="true" className="h-6 w-px shrink-0 self-center bg-outline-variant" />
        <button
          type="button"
          onClick={openPicker}
