...
// Setiap tab memiliki data pemasukan dan pengeluaran
const CHART_DATA = {
  hariIni: {
...
export default function Dashboard({
  ...
  const toggleShopStatus = () => {
    ...
    if (onShopStatusChange) {
      onShopStatusChange(nextStatus)
    }
  }
  ...
  if (showSidebar) {
    return (
      <div className={['flex min-h-screen bg-surface', className].filter(Boolean).join(' ')}>
        <Sidebar activeItemId="dashboard" onLogout={onLogoutClick} />
  ...
  return <div className={className}>{dashboardContent}</div>
}
